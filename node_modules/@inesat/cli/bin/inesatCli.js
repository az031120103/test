...
program
  ...
  // .option('-rm, --remote', '在执行命令时拉取远程声明文件，远程地址首选 package.json 里的 remoteBaseUrlList')
  .option('-ps, --progress <progress>', '显示进度 默认为 true')
  .option('-pr, --profile', '统计模块消耗')
  .option('-cl, --clearLog <clearLog>', '清空日志 默认为 true')
  .option('-wl, --wplogger [filename]', '打印 webpack 配置 默认为 false, filename 为 输出 webpack 配置文件')
  .action(async o => {
...
program
  .command('init')
  .description('初始化 inesat 项目')
  // .option('-t, --template <template>', '模版文件 URL')
  .option('-d, --data [data]', 'JSON 数据 http 地址 或者 文件路径相对、绝对路径')
  .action(o => {
